...
#' @param targetlabel label_id with integer format
...
element_count<- function(slice_no, targetlabel, slice_img1, slice_img2) {
  ...
  if (sum(slice_img1 + slice_img2) > 0) {
    ...
    slice_output <- tibble::tibble(slice_No = slice_no, label_id = targetlabel, overlap_counts = sum(slice_tot == 3), only_file1 = sum(slice_tot == 1), only_file2 = sum(slice_tot == 2), file1_counts = sum(slice_img1 == targetlabel), file2_counts = sum(slice_img2 == targetlabel))
    return(slice_output)
  }
}
